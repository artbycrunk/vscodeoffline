...
class VSCUpdateDefinition(VSCBase):
    ...
    def check_for_update(self, old_commit_id=None):
        ...
        self.updateurl = jresult.get("url")
        self.name = jresult.get("name")
        self.version = jresult.get("version")
        self.productVersion = jresult.get("productVersion")
        self.hash = jresult.get("hash")
        self.timestamp = jresult.get("timestamp")
        self.sha256hash = jresult.get("sha256hash")

        if "supportsFastUpdate" in jresult:
            self.supportsFastUpdate = jresult.get("supportsFastUpdate")
        if self.updateurl:
            return True
        return False
...
class VSCExtensionDefinition(VSCBase):
    def __init__(self, identity, raw=None):
        ...
        if raw:
            ...
            if "extensionId" in raw:
                self.extensionId = raw.get("extensionId")
    ...
    def version(self):
        if self.versions and len(self.versions) > 1:
            log.warning(
                f"version(). More than one version returned for {self.identity}. Unhandled."
            )
            return None
    ...
    def _get_asset_source(self, name):
        if self.versions and len(self.versions) > 1:
            log.warning(
                f"_get_asset_source(). More than one version returned for {self.identity}. Unhandled."
            )
            return None
        for asset in self.versions[0]["files"]:
...
class VSCMarketplace(VSCBase):
    ...
    def _query_marketplace(
        ...
        while count <= total:
            ...
            if "results" in jresult:
                for jres in jresult.get("results"):
                    for extension in jres.get("extensions"):
                        publisherName = extension.get("publisher").get("publisherName")
                        identity = (
                            publisherName + "." + extension.get("extensionName")
                        )
    ...
    def _query(self, filtertype, filtervalue, pageNumber, pageSize):
        return {
            "assetTypes": [],
            "filters": [
                self._query_filter(filtertype, filtervalue, pageNumber, pageSize)
            ],
    ...
    def _query_filter(self, filtertype, filtervalue, pageNumber, pageSize):
        result = {
            ...
            "criteria": [
                ...
                self._query_filter_criteria(
                    vsc.FilterType.ExcludeWithFlags,
                    str(int(vsc.QueryFlags.Unpublished)),
                ),
            ],
        }
    ...
    def _query_flags(self):
        ...
        return (
            vsc.QueryFlags.IncludeFiles
            | vsc.QueryFlags.IncludeVersionProperties
            | vsc.QueryFlags.IncludeAssetUri
            | vsc.QueryFlags.IncludeStatistics
            | vsc.QueryFlags.IncludeStatistics
            | vsc.QueryFlags.IncludeLatestVersionOnly
        )

    def _headers(self, version="1.34.0"):
        insider = "-insider" if self.insider else ""
        return {
...
def run(config):
    ...
    if config.checkspecified:
        ...
        specified = mp.get_specified(specifiedpath) or []
    ...
    if config.checkextensions:
        ...
        recommended = mp.get_recommendations(abs_artifactdir)
    ...
    if config.updatemalicious:
        ...
        malicious = mp.get_malicious(abs_artifactdir, extensions)
    ...
    if config.updateextensions:
        ...
        for identity in extensions:
            ...
            ext_iden = extensions.get(identity)
            ext_iden.download_assets(config.artifactdir_extensions)
            bonus += ext_iden.process_embedded_extensions(
                config.artifactdir_extensions, mp
            )
            ext_iden.save_state(config.artifactdir_extensions)
            count = count + 1
    ...
    log.info("Complete")
    VSCUpdates.signal_updated(abs_artifactdir)
...
def main():
    ...
    abs_artifactdir = os.path.abspath(config.artifactdir)
    config.artifactdir_installers = os.path.join(abs_artifactdir, "installers")
    config.artifactdir_extensions = os.path.join(abs_artifactdir, "extensions")
